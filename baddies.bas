...
SUB DROP_ALIENS()
  ...
  FOR DIDX=0 TO 49
    IF ALIENS(DIDX).STATE=1 THEN
      ...
      IF ALIENS(DIDX).Y > ALIEN_BOTTOM THEN ALIEN_BOTTOM = ALIENS(DIDX).Y
      IF ALIENS(DIDX).Y < ALIEN_TOP THEN ALIEN_TOP = ALIENS(DIDX).Y
...
SUB MOVE_ALIENS()
  ...
  REM include ALIEN_BOTTOM row - was clearing one row too few (trail below formation)
  MEMSET SCREENADDRESS+((ALIEN_TOP-1)*40), (((ALIEN_BOTTOM-ALIEN_TOP)+2)*40), 32
...
ALIEN_DIRECTION=1: ALIEN_LEFT=-1: ALIEN_RIGHT=37
