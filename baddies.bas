...
SUB MOVE_ALIENS()
  ...
  MEMSET SCREENADDRESS, 400, 32
  FOR IDX=0 TO 49
    IF ALIENS(IDX).STATE=1 THEN
    
      POKE SCREENADDRESS+(40*ALIENS(IDX).Y)+ALIENS(IDX).X,32
      ALIENS(IDX).X=ALIEN_BASE+ALIENS(IDX).X+ALIEN_DIRECTION
