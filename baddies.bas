...
SUB MOVE_ALIENS()
  FOR I=0 TO 49
    ...
    IF ALIENS(I).STATE=1 THEN
      POKE SCREENADDRESS+(40*ALIENS(I).Y)+ALIENS(I).X,32
      ALIENS(I).X=ALIENS(I).X+ALIEN_DIRECTION
      IF ALIENS(I).X<ALIEN_LEFT THEN ALIEN_LEFT=ALIENS(I).X
      IF ALIENS(I).X>ALIEN_RIGHT THEN ALIEN_RIGHT=ALIENS(I).X
